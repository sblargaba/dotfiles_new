...
function run_once(cmd)
  ...
  awful.spawn.with_shell("pidof " .. findme .. " > /dev/null || (" .. cmd .. ")")
 end
...
-- {{{ Localization
-- os.setlocale(os.getenv("it_IT.UTF-8"))
-- }}}
...
globalkeys = awful.util.table.join(
    ...
    awful.key({ modkey }, "b", function () awful.spawn("firefox") end,
              {description = "starts Firefox", group = "launcher"}),
    awful.key({ modkey }, "s", function () awful.spawn("pcmanfm") end,
              {description = "starts file manager", group = "launcher"}),
    awful.key({ modkey, "Shift"   }, "p", function () awful.spawn("firefox -private-window") end,
              {description = "starts Firefox in private mode", group = "launcher"})
)
...
function notify(id, val)
    if type(val) ~= "number" then
        ids[id] = naughty.notify({
            title = id .. val,
            replaces_id = ids[id]
        }).id
    else
        local out = { "[" }
        local width = string.len(id) + 1
        local bars = math.ceil(val * width / 100)
        local spaces = width - bars
        for i = 2, bars + 1 do out[i] = "-" end
        for i = bars + 2, width + 1 do out[i] = " " end
        ids[id] = naughty.notify({
            title = id .. val .. "%",
